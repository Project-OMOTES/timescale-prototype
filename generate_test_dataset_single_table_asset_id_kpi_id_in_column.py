...
# Doesn work! Postgresql only accepts 1600 columns per table.

NUM_OF_ASSETS = 250
KPIs = ["HeatIn.Q1", "Heat_flow1", "PostProc.Velocity1", "HeatIn.Q2", "Heat_flow2",
        "PostProc.Velocity2", "HeatIn.Q3", "Heat_flow3", "PostProc.Velocity3", "HeatIn.Q4"]
...
''')
    ...
    for asset_i, asset_id in enumerate(asset_ids):
        ...
        open_asset_metadata_file.write(
            f"    ('{asset_id}', '{random.choice(['HeatingDemand', 'Pipe', 'ResidualHeatSource'])}', 'Asset {asset_i}', '{random.choice(['Consumer', 'Transport', 'Producer'])}', '{uuid.uuid4()}'){end_line}")
...
CREATE TABLE "{esdl_id}_profiles" (
    time TIMESTAMPTZ,
    asset_id TEXT,
    {',\n    '.join(f'"{asset_id}_{kpi}" real' for kpi in KPIs for asset_id in asset_ids)}
);
...
''')
    ...
    while current_time < END_DATETIME:
        rows.append(
            f"('{current_time.isoformat(sep=' ')}', '{asset_id}', {', '.join(str(random.uniform(0, 10)) for _ in KPIs for _ in asset_ids)})")
