...
''')
    ...
    for asset_i, asset_id in enumerate(asset_ids):
        open_profiles_file.write(f'''CREATE TABLE "{asset_id}" (
                time TIMESTAMPTZ,
                {',\n    '.join(f'"{kpi}" REAL' for kpi in KPIs)}
            );    
    
    INSERT INTO "{asset_id}" VALUES
    ''')
        ...
        while current_time < END_DATETIME:
            ...
            open_profiles_file.write(f"('{current_time.isoformat(sep=' ')}', {', '.join(str(random.uniform(0, 10)) for _ in KPIs)}){end_line}")
